...
angular.module('app').controller('familyZoomChartCtrl', ["$scope", "$http", function ($scope, $http) {
	...
	$http.get('./dist/html/familyZoomChart/testFamily.json').then(function (response) {
		...
		function buildData(rootPerson, rootPersonArray) {
			...
			var rootsDad = _.find(family, function (person) {
				return rootPerson.descendancyNumber && rootPerson.descendancyNumber[0] === person.ascendancyNumber;
			});
			...
			if (rootPerson.hasOwnProperty('children')) {
				...
				rootPerson.children.forEach(function (child) {
					if (child.descendancyNumber.length === 2 && child.descendancyNumber.indexOf('S') > -1) {
						child.spouse = true;
						rootPersonSpouse = _.find(family, function (spouse) {
							return spouse.personId === child.personId;
						});
					} else {
