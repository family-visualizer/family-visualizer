...
angular.module('app').directive('lifespanChart', ["$parse", "$window", function ($parse, $window) {
	...
	return {
		...
		link: function link(scope, elem, attrs) {
			...
			var x = d3.scale.linear().domain([1900, 2014]).range([0, width]).clamp(true);

			// defines brush
			var brush = d3.svg.brush().x(x).extent([startingValue, startingValue]).on("brush", brushed);
			...
			.call(d3.svg.axis().scale(x).orient("bottom").tickFormat(function (d) {
				return d;
			}).tickSize(0).tickPadding(12).tickValues([1900, 2014])).select(".domain").select(function () {
			...
			handle.append("path").attr("transform", "translate(0," + height / 2 + ")").attr("d", "M 0 -20 V 20");
