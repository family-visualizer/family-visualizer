	...
	this.flattenTree = function (familyTree) {
		...
		flattenedFamily.forEach(function (person) {
			if (addedIds.indexOf(person.personId) === -1) {
				...
				// var existing = _.find(uniqueFlattened, function(p){ return p.personId === person.personId; });
			...
			// If the cleanPerson matches someone in combined family...
			// if (cleanPerson.personId === person.personId) {
			// 	console.log('match found', person.name);

			// 	//If that person has an ascendancy number, assign that to their clean version
			// 	if (person.ascendancyNumber) {
			// 		cleanPerson.ascendancyNumber = person.ascendancyNumber;
			// 	}
			// 	//If that person has a descendancy number, assign that to their clean version
			// 	if (person.descendancyNumber) {
			// 		cleanPerson.descendancyNumber = person.descendancyNumber;
			// 	}
			// }
		});
				...
				allTheDads.forEach(function (person, personIndex) {
					...
					makeAncestryAndChildrenCall.call(this, accessToken, defer, person).then(function (response) {
						...
						response.data.persons.forEach(function (child, childIndex) {
							...
							// if (personIndex === self.ancestry.length - 1 && childIndex === response.data.persons.length -1) {
...
angular.module('app').controller('userHomeCtrl', ["$scope", "authService", "familyService", function ($scope, authService, familyService) {
	...
	var getAncestryAndChildren = function getAncestryAndChildren() {
		familyService.getAncestryAndChildren($scope.authedUser.personId, $scope.authedUser.accessToken).then(function (data) {
			$scope.family = data;
			console.log('$scope.ancestryAndChildren:', $scope.ancestryAndChildren);
			// cleanData();
		});
	};

	// function cleanData() {
	// 	console.log('length', $scope.ancestryAndChildren.length);
	// 		for (var i = 0; i < $scope.ancestryAndChildren.length; i++) {
	// 		console.log('pushing PARENT', $scope.ancestryAndChildren[i].name);
	// 		// $scope.combinedFamily.push(person);
	// 			for (var j = 0; j < $scope.ancestryAndChildren[i].children.length; j++) {
	// 				console.log('pushing child', $scope.ancestryAndChildren[i].children[j].name);
	// 				// child.descendancyNumber = person.ascendancyNumber + child.descendancyNumber.slice(1, 3);
	// 				// $scope.combinedFamily.push(child);
	// 			}
	// 		}
	// }
}]);
...
angular.module('app').controller('lifespanChartCtrl', ["$scope", "familyService", function ($scope, familyService) {
	...
	// $scope.familyTest = familyService.returnAncestryAndChildren();

	//familyDataUtilsService.flattenTree($scope.family);

	// $scope.combineAncestorsAndChildren = function () {

	// 	$scope.family.forEach(function (person) {
	// 		console.log('pushing PARENT', person.name);
	// 		$scope.combinedFamily.push(person);
	// 		person.children.forEach(function (child) {
	// 			console.log('pushing child', child.name);
	...
	// 	console.log('combined family', $scope.combinedFamily);

	// 	$scope.cleanedFamily = _.uniq($scope.combinedFamily, function (person) {
	// 		return person.personId;
	// 	});

	// 	$scope.cleanedFamily.forEach(function (cleanPerson) {
	// 		$scope.combinedFamily.forEach(function (person) {
	// 			// If the cleanPerson matches someone in combined family...
	// 			if (cleanPerson.personId === person.personId) {
	// 				console.log('match found', person.name);
	...
	// 	console.log('clean family', $scope.cleanedFamily);
	// };

	// $scope.combineAncestorsAndChildren();
...
angular.module('app').directive('lifespanChart', ["$parse", "$window", function ($parse, $window) {
	...
	return {
		...
		link: function link(scope, elem, attrs) {
			...
			svg.append("rect").attr('width', "100%").attr('height', "100%").attr('fill', 'rgb(100,0,222)').attr('margin', 50);
			...
			.duration(2000).attr({
				...
				fill: function fill(data, sliderValue) {
